...
グループ登録後は、ディレクトリを追加するだけで即座にアクセス可能。明示ルートの変更時は Apache の graceful restart で反映される（1秒未満、既存接続を中断しない）。
...
| レイヤ | 技術 | 理由 |
| --- | --- | --- |
| ルーティングエンジン | 名前ベース VirtualHost（自動生成）+ mod_vhost_alias | 明示ルートは個別 VirtualHost、グループはワイルドカード VirtualHost + VirtualDocumentRoot で動的解決 |
| 管理 API バックエンド | PHP（Apache 直接実行） | フレームワーク不要・外部依存ゼロ |
| フロントエンド（管理 UI） | HTML / CSS / Alpine.js | ビルドステップ不要。Apache が直接配信する静的ファイルとして完結 |
| データ永続化 | routes.json + routes.conf / routes-ssl.conf（自動生成） | JSON で管理し、変更時に VirtualHost 定義ファイルを再生成 + graceful restart。グループ内プロジェクト追加時は VirtualDocumentRoot による動的解決のため再起動不要 |

技術選定の経緯は[意思決定記録](../decisions/index.md)を参照。
